...
function ProjectCard({ project, index }) {
  ...
  return (
    <motion.div initial={{ opacity:0, y:60 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:0.8, delay:index*0.15 }}
      ...
      {/* Hero */}
      <div className="relative h-52 flex items-center justify-center overflow-hidden group-hover:shadow-inner" style={{ background:project.bgGrad }}>
        ...
        <motion.span className="text-6xl relative z-10" animate={hov ? { scale:[1,1.12,1.06], rotate:[0,-4,4,0], opacity: project.image ? 0 : 1 } : { scale:1, rotate:0, opacity: 1 }} transition={{ duration:0.5 }}>{project.icon}</motion.span>
        
        <div className="absolute top-3 right-3 z-20"><span className="font-mono text-xs px-2 py-1 rounded-full" style={{ color:project.accent, background:`${project.accent}18`, border:`1px solid ${project.accent}33`, backdropFilter: "blur(4px)" }}>Featured</span></div>
        <div className="absolute bottom-0 left-0 right-0 h-16 z-10" style={{ background:"linear-gradient(to top, rgba(6,6,8,0.9), transparent)" }} />
      </div>
      {/* Body */}
